...
if [ "x$help" = "xhelp" -o "x$errors" != "x" ] ; then
    ...
    echo "    --haplo-divergence       Estimation on maximum divergence between haplotypes, is used only with Hi-C/PoreC data."
    echo "                             Should be increased for species with divergence significantly higher than in human. Default: 0.05, min 0, max 0.2"
    echo ""
    ...
    echo ""
    echo "    --paths <paths.gaf>      Option required to generate fasta file from user-provided paths through assembly graph."
    echo "                             Requires an existing assembly specified by '--assembly <path_to_assembly>' and input reads."
    echo "                             Output is specified as '-d <output_dir>' (<output_dir> should  not be equal to <path_to_assembly>)."
    echo "                             The gaf file must be formatted as follows: 'name >utig4-1<utig4-2 HAPLOTYPE1' with one path per line,"
    echo "                             where utig4-1 is in fwd orientation and utig4-2 is in reverse complement."
    echo ""
