...
#                    names = read.reference_name
        ...
        if all_middle[0] or all_middle[1]:
            one_middle_total += 1
        if unique or ((not (all_short[0] or all_short[1])) and (not (all_middle[0] or all_middle[1]))):
...
sys.stderr.write (f"For pairs addfiltered: Both short {both_short_total} one short {one_short_total} both middle {both_middle_total} one middle {one_middle_total}\n")